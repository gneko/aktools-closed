...
export class DetectSetttingComponent implements OnInit {
  ...
  get ModifyingConfidence() {
    return (this.ModifyingItem.item.find((v) => v.id == this.ModifyBuffer.id) || ({ confidence: 0 })).confidence;
  }
  Merge() {
    ...
    this.EditGuiReset();
    this.worker.postMessage({ method: 'calcDhash', ImageDatas: this.ImageDatas, index: this.XBound.length * this.Modifying.y + this.Modifying.x }); //重算dHash
  }
  Replace() {
    if (this.ModifyBuffer.id.toString() == "0000") {
      return;
